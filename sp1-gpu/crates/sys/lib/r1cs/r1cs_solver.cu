// In-process GPU R1CS solver for the SP1 Groth16 wrap helper.
//
// Mirrors crates/recursion/gnark-ffi/r1cs_solver_proto/full_solve_warp_prod.cu
// but exposes a stable C ABI (sp1_r1cs_solver_*) that the helper Rust
// code can call without spawning a subprocess. Holds circuit data on
// GPU across calls, so per-prove cost is dominated by the kernel
// itself (~1.2 s on 5090) rather than CUDA init + uploads (~3.5 s).
...
sp1_r1cs_solver_create(const char* prep_circuit_dir,
    ...
    int sm_count = 0, mbpm = 0;
    cudaDeviceGetAttribute(&sm_count, cudaDevAttrMultiProcessorCount, 0);
    int block = 256;
    if (const char* env = getenv("SP1_R1CS_BLOCK")) block = atoi(env);
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&mbpm,
        (const void*)persistent_solve_kernel, block, 0);
    if (const char* env = getenv("SP1_R1CS_BLOCKS_PER_SM")) mbpm = atoi(env);
    if (mbpm < 1) mbpm = 1;
    h->grid = sm_count * 1; // bps=1 is the optimum (verified Phase 7+8)
    h->block = block;

    fprintf(stderr,
        "[r1cs-solver] ready: %lu wires, %lu constraints, %u layers; grid=%d block=%d\n",
