// Phase 7 prototype, HIP/RDNA3: warp-cooperative LE accumulate.
// See full_solve_warp.cu for the design rationale (CUDA original).
//
// On RDNA3 (gfx1100):
//   - "warp" = wave32 (one wavefront, 32 lanes). Block size must be a
//     multiple of 32; we use 128 (4 wavefronts/block).
//   - __shfl_xor on RDNA3 lowers to ds_bpermute_b32 (LDS-routed) — see
//     project memory feedback_warpshfl_fuse_nogo.md. Slower than CUDA's
//     register shuffle but still much faster than the 32 separate
//     scalar mul-adds it replaces.
//   - bn254_t::inv() is changed to __noinline__ here as well to drop
//     VGPR pressure (per agent 9 finding: dropping forceinline frees
//     ~10 VGPRs and unlocks 2× occupancy on RDNA3 if we cross the cliff).
//
// Build:
//   hipcc -std=c++20 -O3 -I .../sys/include --offload-arch=gfx1100 \
//         -DUSE_HIP full_solve_warp_hip.cu -o build/full_solve_warp_hip
...
// __noinline__ keeps inv() out of the hot kernel's VGPR footprint.
__device__ __attribute__((noinline)) bn254_t fr_inv(bn254_t a) {
    return a.inv();
}

// Raw AMDGCN intrinsic: ds_bpermute_b32 with XOR-derived source lane.
// Bypasses HIP's __shfl_xor wrapper (suspected buggy lowering on
// gfx1100 / ROCm 7.2 in cooperative-launch context).
__device__ __forceinline__ uint32_t bpermute_xor(uint32_t v, int offset) {
    int lane = __builtin_amdgcn_mbcnt_hi(-1, __builtin_amdgcn_mbcnt_lo(-1, 0));
    // ds_bpermute takes byte address (lane << 2) of the *source* lane.
    int src_addr = ((lane ^ offset) & 31) << 2;
    return __builtin_amdgcn_ds_bpermute(src_addr, (int)v);
}

// Warp-reduction (32 lanes, wave32) of an Fr value using raw bpermute.
__device__ __forceinline__ bn254_t warp_reduce_fr(bn254_t v) {
    for (int offset = 16; offset > 0; offset >>= 1) {
        bn254_t other;
        #pragma unroll
        for (int i = 0; i < 8; ++i) {
            other.data[i] = bpermute_xor(v.data[i], offset);
        }
        v = v + other;
    }
    return v;
...
int main(int argc, char** argv) {
    ...
    fprintf(stderr, "[warp-hip] %u layers, %zu wires\n", nb_layers, n_wires);
    ...
    fprintf(stderr, "[warp-hip] upload: %.1f ms\n",
    ...
    fprintf(stderr, "[warp-hip] sm_count=%d max_blocks/sm=%d grid=%d block=%d (warps=%d)\n",
    ...
    hipError_t err = hipLaunchCooperativeKernel((void*)persistent_solve_warp_kernel,
                                                 dim3(grid), dim3(block), args, 0, 0);
    if (err != hipSuccess) {
        fprintf(stderr, "[warp-hip] launch: %s\n", hipGetErrorString(err));
        return 2;
    }
    HIP_CHECK(hipDeviceSynchronize());
    auto solve_t1 = std::chrono::steady_clock::now();
    double solve_ms = std::chrono::duration<double, std::milli>(solve_t1 - solve_t0).count();
    fprintf(stderr, "[warp-hip] solve: %.1f ms (1 cooperative launch, %d warps)\n",
            solve_ms, n_warps);

    int err_flag = 0;
    HIP_CHECK(hipMemcpy(&err_flag, d_err, sizeof(int), hipMemcpyDeviceToHost));
    if (err_flag != 0) fprintf(stderr, "[warp-hip] verify-only fail: %d\n", err_flag - 1);
    ...
    if (mismatches > 0) {
        fprintf(stderr, "[warp-hip] FAIL — %zu mismatches; first wire %ld\n", mismatches, first_mm);
        return 3;
    }
    if (err_flag != 0) return 4;

    fprintf(stderr, "[warp-hip] PASS — all %zu wires match expected\n", n_wires);
    return 0;
}
